    ...
    const generateListItem = () => {
        ...
        for (let index = 0; index < renderedNodes; index++) {
            ...
            if (dataItem && typeof dataItem === 'object') {
                const dataItemId =
                    'id' in dataItem &&
                    (typeof dataItem.id === 'number' || typeof dataItem.id === 'string')
                        ? dataItem.id
                        : null;
                if (dataItemId && transactions[dataItemId]) {
                    items.push(<ListComponent key={dataItemId} data={transactions[dataItemId]} />);
                }
            }
        }
        return items;
