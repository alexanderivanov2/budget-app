...
const TransactionListDay: React.FC<Props> = ({ dayTransactions }) => {
    ...
    const descDayTransactions = [...dayTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())]
    return (
        <div className="day-wrapper">
            {descDayTransactions.map((dayTransact) => {
                const transaction = transactions[dayTransact.id];
                return (
                    <div className={`statistic-transaction-item ${transaction.type}`} key={dayTransact.id}>
